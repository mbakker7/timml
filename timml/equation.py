...
class IntDisVecEquation:
    def equation(self):
        ...
        for icp in range(self.ncp):
            ...
            for e in self.model.elementlist:
                if e.nunknowns > 0:
                    flux = self.intflux(e.disvecinflayers, self.xc[icp], self.yc[icp],
                                        self.xc[icp + 1], self.yc[icp + 1], self.layers, aq=self.aq)
                    mat[istart:istart + self.nlayers, ieq:ieq + e.nunknowns] = flux
                    ieq += e.nunknowns
                else:
                    flux = self.intflux(e.disveclayers, self.xc[icp], self.yc[icp],
                                        self.xc[icp + 1], self.yc[icp + 1], self.layers, aq=self.aq)
