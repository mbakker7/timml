...
def param_maq(kaq, z, c, npor, top):
    ...
    H = z[:-1] - z[1:]
    assert np.all(H >= 0), "Error: Not all layers thicknesses are non-negative " + str(
        H
    )
    if top == "conf":
        if len(c) == 1:
        ...
        assert len(npor) == 2 * Naq - 1, (
            "Error: Length of npor needs to be 1 or " + str(2 * Naq - 1)
        )
        c = np.hstack((1e100, c))
    else:  # leaky layer on top
        if len(c) == 1:
...
def param_3d(kaq, z, kzoverkh, npor, top="conf", topres=0):
    ...
    assert len(kzoverkh) == Naq, "Error: length of kzoverkh needs to be 1 or " + str(
        Naq
    )
    if len(npor) == 1:
        if top == "conf":
